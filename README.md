...
Gemini 3.6, 3.7 and 3.8 Flash are on a promotional rate that Google bills through 2026-12-31, after which the list price applies (double the promotional rate). Those entries carry `promo_until` plus their list prices, and `config.py` swaps the list price in automatically once the date passes — no edit needed in January.
